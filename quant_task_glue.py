...
import numpy as np
import torch
from torch.nn import CosineSimilarity, CrossEntropyLoss, MSELoss
...
def main():
    ...
    parser.add_argument(
        "--teacher_model", default=None, type=str, help="The models directory."
    )
    parser.add_argument(
    ...
    parser.add_argument("--batch_size", default=None, type=float, help="batch_size")
    parser.add_argument(
        "--seed", type=int, default=42, help="random seed for initialization"
    )
    parser.add_argument(
        "--aug_train", action="store_true", help="Whether to use augmented data or not"
    )

    parser.add_argument(
        "--pred_distill", action="store_true", help="Whether to distill with task layer"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--value_distill", action="store_true", help="Whether to distill with value"
    )
    parser.add_argument(
        "--context_distill", action="store_true", help="Whether to distill with context"
    )
    parser.add_argument(
        "--att_distill",
        action="store_true",
    ...
    parser.add_argument(
        "--query_distill", action="store_true", help="Whether to distill with query"
    )
    parser.add_argument(
        "--key_distill", action="store_true", help="Whether to distill with key"
    )

    parser.add_argument(
        "--save_fp_model", action="store_true", help="Whether to save fp32 model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--clip_val", default=2.5, type=float, help="Initial clip value."
    )
    parser.add_argument(
        "--do_eval", action="store_true", help="Whether to run eval on the dev set."
    )
    parser.add_argument(
        "--omega", default=1, type=float, help="coeff of value attention distillation."
    )
    parser.add_argument(
        "--ground_truth",
        action="store_true",
    ...
    if not args.do_eval:
        assert (
            args.pred_distill or args.intermediate_distill
        ), "'pred_distill' and 'intermediate_distill', at least one must be True"
    ...
    if not args.do_eval:
        ...
        num_train_optimization_steps = (
            int(len(train_features) / args.batch_size) * args.num_train_epochs
        )
    ...
    if task_name in corr_tasks:
        fp32_performance = (
            f"pearson/spearmanr:{result['pearson']}/{result['spearmanr']}"
        )
            ...
            for step, batch in enumerate(
                ...
                with torch.no_grad():
                    (
                        ...
                        teacher_queries,
                        teacher_keys,
                    ) = teacher_model(input_ids, segment_ids, input_mask, epoch=step)
                ...
                if args.pred_distill:
                    if output_mode == "classification":
                        ...
                        gt_beta = 0.5
                        logsoftmax_student_logits = torch.nn.functional.log_softmax(
                ...
                def direction_matching_distillation(student_scores, teacher_scores):
                    tmp_loss = 0.0
                    new_teacher_scores = [
                        teacher_scores[i * layers_per_block + layers_per_block - 1]
                        for i in range(student_layer_num)
                    ]
                    for student_score, teacher_score in zip(
                        student_scores, new_teacher_scores
                    ):
                        student_score = torch.where(
                            student_score <= -1e2,
                            torch.zeros_like(student_score).to(device),
                            student_score,
                        )
                        teacher_score = torch.where(
                            teacher_score <= -1e2,
                ...
                if args.query_distill:
                    query_loss = direction_matching_distillation(
                ...
                if args.key_distill:
                    key_loss = direction_matching_distillation(
                ...
                if args.value_distill:
                    value_loss = direction_matching_distillation(
                ...
                nb_tr_steps += 1
                if (
                    global_step % args.eval_step == 0
                    or global_step == num_train_optimization_steps - 1
                ):
                    logger.info("***** Running evaluation *****")
                    logger.info(
                        "  Epoch = {} iter {} step, ({} steps in total)".format(
                            epoch_, global_step, num_train_optimization_steps
                        )
                    )
                    logger.info("  Num examples = %d", len(eval_examples))
                    logger.info("  Batch size = %d", args.batch_size)
                            ...
                            for name, module in quant_model.named_modules():
                                if hasattr(module, "weight_quantizer"):
                                    if module.weight_bits == 1:
                                        module.weight.data = (
                                            module.weight_quantizer.apply(module.weight)
                                        )
                                    else:
                                        module.weight.data = (
                                            module.weight_quantizer.apply(
                                                module.weight,
